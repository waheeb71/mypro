"""
Enterprise CyberNexus — WAF Preprocessor
...
class WAFPreprocessor:
    ...
    def _strip_sql_comments(self, text: str) -> Tuple[str, bool]:
        """
        Lexical scanner to remove SQL comments while respecting string literals.
        Acts as a lightweight AST tokenizer to stop obfuscation like UNION/**/SELECT 
        without breaking actual data strings.
        """
        result = []
        i = 0
        length = len(text)
        ...
        while i < length:
            ...
            if c == '/' and i + 1 < length and text[i+1] == '*':
                changed = True
                i += 2
                while i < length and not (text[i-1] == '*' and text[i] == '/'):
                    i += 1
                i += 1
                result.append(' ') # replace with space to prevent concatenation
                continue

            # Handle line comments -- or #
            if (c == '-' and i + 1 < length and text[i+1] == '-') or (c == '#'):
                changed = True
