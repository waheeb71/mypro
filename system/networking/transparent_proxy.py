#!/usr/bin/env python3
"""
Enterprise NGFW - Dynamic Transparent Proxy Configuration Manager
...
class TransparentProxyManager:
    ...
    traffic based on the NGFW configuration.
    """
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        for k, v in port_mappings.items():
            if str(k) == "443" and "transparent" in str(v):
                # Configuration doesn't directly tell what the intercept port is, assuming standard NGFW ports
                pass
    ...
    def clear_existing_rules(self):
        """Clears all old NGFW_REDIRECT chains and rules."""
        if os.geteuid() != 0:
            logger.warning("Not root. Cannot modify iptables.")
            return
            
        logger.info("Cleaning up old NGFW iptables rules...")
        self._execute_iptables(["-t", "nat", "-D", "PREROUTING", "-j", "NGFW_REDIRECT"], ignore_errors=True)
        self._execute_iptables(["-t", "nat", "-F", "NGFW_REDIRECT"], ignore_errors=True)
        self._execute_iptables(["-t", "nat", "-X", "NGFW_REDIRECT"], ignore_errors=True)
        
        self._execute_iptables(["-D", "FORWARD", "-j", "NGFW_FORWARD"], ignore_errors=True)
        self._execute_iptables(["-F", "NGFW_FORWARD"], ignore_errors=True)
        self._execute_iptables(["-X", "NGFW_FORWARD"], ignore_errors=True)
    ...
    def setup_transparent_rules(self):
        ...
        try:
            # Create Custom Chain
            self._execute_iptables(["-t", "nat", "-N", "NGFW_REDIRECT"])
            
            # Exclude local traffic bounds
            for subnet in ["127.0.0.0/8", "169.254.0.0/16", "224.0.0.0/4"]:
                self._execute_iptables(["-t", "nat", "-A", "NGFW_REDIRECT", "-d", subnet, "-j", "RETURN"])
            
            # By default, do not inspect traffic generated by the root proxy process itself
            # We assume the python proxy runs as root or a dedicated 'ngfw' user.
            uid = os.geteuid()
            self._execute_iptables(["-t", "nat", "-A", "NGFW_REDIRECT", "-m", "owner", "--uid-owner", str(uid), "-j", "RETURN"], ignore_errors=True)

            # Redirect Port 80
            self._execute_iptables(["-t", "nat", "-A", "NGFW_REDIRECT", "-p", "tcp", "--dport", "80", 
                                  "-j", "REDIRECT", "--to-ports", str(self.http_redirect_port)])
                                  
            # Redirect Port 443
            self._execute_iptables(["-t", "nat", "-A", "NGFW_REDIRECT", "-p", "tcp", "--dport", "443", 
                                  "-j", "REDIRECT", "--to-ports", str(self.https_redirect_port)])

            # Attach to PREROUTING for the LAN interface
            self._execute_iptables(["-t", "nat", "-I", "PREROUTING", "-i", self.lan_interface, "-j", "NGFW_REDIRECT"])
    ...
    def teardown(self):
        ...
        logger.info("✅ NGFW Network rules reverted.")
